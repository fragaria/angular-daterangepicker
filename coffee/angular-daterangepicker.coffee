...
picker.directive 'dateRangePicker', ($compile, $timeout, $parse, $q, $injector, dateRangePickerConfig) ->
  require: 'ngModel'
  restrict: 'A'
  scope:
    min: '='
  ...
  link: ($scope, element, attrs, modelCtrl) ->
    ...
    _config = dateRangePickerConfig
    if _config.provider
      _config = $injector.get(_config.provider)()
    $q.when(_config).then(_doLink)
